...
class AuthServices {
  ...
  static async resendVerification(email) {
    try {
      const response = await axios.post(
        `${AuthServices.BASE_URL}api/verification/resend`,
         email , 
        this.getAuthHeaders()
      );
      return response.data;
  ...
  static async verifyOtp(data){
    try{
      const response = await axios.post(`${AuthServices.BASE_URL}api/verification/verify-email`, data, this.getAuthHeaders());
      return response.data;
    } catch (err){
      throw err;
    }
